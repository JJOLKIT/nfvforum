...
public class Function {
	 ...
	 * @return
	 */
	public static String checkZero(int arg) {
		String r = "";
		if (arg == 0) {
			r = "";
		} else {
	...
	public static int getIntParameter(String Req ) {
		try {
			if (Req == null || Req.equals("") || Req.equals("null")) {
				return 0;
			} else {
				Req = Req.replaceAll(",", "");	// , 제외
				if (Req.indexOf(".")>-1) Req = Req.substring(0, Req.indexOf("."));	// 소수점이 있는 경우 정수까지만 자름
	...
	public static int getIntParameter(Object arg ) {
		String Req = parseStr(arg);
		try {
			if (Req == null || Req.equals("") || Req.equals("null")) {
				return 0;
			} else {
	 ...
	 * @return size만큼의 문자열
	 */
	public static String getHtmlRemoveTextReSize(Object obj, int size) {
		String html = parseStr(obj);
	...
	public static String getSslCheckUrl() {
		...
		String url = "";
		if (SiteProperty.SSL_USE) {
			url = SiteProperty.COMPANY_SSL_URL;
		} else {
			url = SiteProperty.COMPANY_URL;
		}
		return url;
	}
	...
	public static String getRemoviSslUrl() {
		String url = SiteProperty.COMPANY_URL;
		return url;
	}
	...
	public static boolean checkReferer(String referer) {
		...
		if (SiteProperty.getCHECK_REFERER()) {
			if (referer.indexOf(SiteProperty.getREFERER_URL()) > 0) {
				result = true;
			}
		} else {
			result = true;
	...
	/**
	 * 소수점 두자리까지 표기, 0은 표기안하도록
	 * @param var
	 * @return
	 */
	public static String getDecimalFormat(double var) {
		DecimalFormat df = new DecimalFormat("#.##");
	 ...
	 * @return
	 */
	public static String getHundredMilion(int won) {
		...
		if (hm >= 1) {
			result = d.format(hm)+"억";
		}
		if (hm_mod >= 1) {
			result += " "+d.format(hm_mod)+"만";
		}
		
		if ("".equals(result)) result = "0만"; 
        result += "원";
        return result;
    }
	
	public static String getHundredMilion(long won) {
		DecimalFormat d = new DecimalFormat("#,###");
		String result = "";
		long hm = 0;
		long hm_mod = 0;
	...
	/**
	 * 썸네일 이미지 생성 (기존파일명앞에 thumb_를 붙여 생성)
	 * @param uploadPath 경로
	 * @param filename 원본파일
	 * @param width
	 * @throws Exception
	 */
	public static void resizeImage(String uploadPath, String filename, int width) throws Exception {
		
		if (!"".equals(Function.checkNull(filename))) {	// 첨부된 파일이 있는 경우에만 실행
			String file = uploadPath+"/"+filename;
			String targetFile = uploadPath+"/thumb_"+filename;
			Image imgSource = new ImageIcon(file).getImage();
			int	w = imgSource.getWidth(null);
			if (w > width) {	// 실제 이미지파일이 리사이징 사이즈보다 클경우만 실행
				imageUtil.Image image = ImageLoader.fromFile(file);
				image.getResizedToWidth(width).soften(0.08f).writeToJPG(new File(targetFile), 0.8f);
			} else {
				File f = new File(file);
				f.renameTo(new File(targetFile));	// 실제 이미지파일 사이즈가 작을 경우 파일명만 변경
			}
		}
	}
	
	/** 
	 * 배열의 정렬순서에 따른 index값 리턴 (오름차순)
	 * @param type
	 * @return
	 */
	public static int[] cmcSort(final int[] type) {
		final Integer[] s = new Integer[type.length];
		for (int i=0; i<type.length; i++) s[i] = i;
		
		Arrays.sort(s, new Comparator<Integer>() {

			@Override
			public int compare(Integer arg0, Integer arg1) {
				return Integer.compare(type[arg0], type[arg1]);
			}
			
		});
		
		int[] r = new int[type.length];
	...
	public static String stringComma(String num) {
		String arr1 = "";
		String arr2 = "";
		if (num.indexOf(".") > -1) {
			arr1 = num.split("\\.")[0];
		...
		DecimalFormat df = new DecimalFormat("##,###");
		String temp = df.format(Integer.parseInt(arr1)) + "." + arr2;
		return temp;
	}
	...
	public static int getStartPageNo(int reqPageNo, int pageRows) {
		int startPageNo = 0;
		if (reqPageNo > 0) {
	...
	public static int parseInt(Object obj) {
		...
		if (obj != null) {
			...
			if ("null".equals(str)) rst = 0;
			else rst = getIntParameter(str);
		}
		return rst;
	}
	...
	public static String parseDate(Object obj) {
		String rst = "";
		if (obj != null) {
			rst = String.valueOf(obj).substring(0,10);
			if ("null".equals(rst)) rst = "";
		}
		return rst;
	}
	
	public static String parseDateTime(Object obj) {
		String rst = "";
		if (obj != null) {
			rst = String.valueOf(obj).substring(0,19);
			if ("null".equals(rst)) rst = "";
		}
		return rst;
	}
	...
	public static int getMobileCheck(HttpServletRequest request) {
...
//		System.out.println(strCut("본건물은 채무액을 상환할 목적으로...", "", 30, 0, true, true));
//		System.out.println(message(1, "참", "거짓"));
//		System.out.println(getHundredMilion(12000));
		System.out.println(decimalFormat(1234.3400000000));
		System.out.println(stringComma("1234.34"));
		//String[] a = "1234.56".split("\\.");
