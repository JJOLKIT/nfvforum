...
<%
	...
	String vf2 = java.net.URLEncoder.encode(af, "UTF-8");		// 다시 UTF-8로 인코딩 필요
	
	
	//String vf2 = vf;
	path = request.getRealPath(path+vf); 			 			// 로컬
	...
	try {
		...
		if(strClient.indexOf("MSIE 5.5")>-1) {
			...
			response.setHeader("Content-Disposition", "filename=" + vf2.replaceAll("\\+", "%20") + ";");
		} else if(strClient.indexOf("Trident") >= 0) {
			response.setHeader("Content-Type", "application/octet-stream");
			//response.setHeader("Content-Disposition", "attachment;filename=" + (java.net.URLEncoder.encode(vf2, "8859_1").replaceAll("\\+", "%20")) + ";");
			response.setHeader("Content-Disposition", "attachment;filename=" + vf2.replaceAll("\\+", "%20") + ";");
		} else {
			response.setHeader("Content-Type", "application/octet-stream");
			response.setHeader("Content-Disposition", "attachment;filename=" + vf2.replaceAll("\\+", "%20") + ";");
		}
