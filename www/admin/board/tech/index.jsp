			...
			<table class="searchTable">
				...
				<tbody>
					<tr>
						...
						<th class="bno">카테고리</th>
						<td class="bno">
							<select name="scategory" id="scategory" onchange="goSearch();">
							<%=CodeUtil.getTechCategoryOption(param.get("scategory")) %>
							</select>
						</td>
						<th class="bno">검색어</th>
						<td class="bno">
			...
			<table>
				...
				%>
					<tr>
						...
						<td <%=targetUrl%>>
							<%=CodeUtil.getTechCategoryName(Function.parseInt(data.get("category"))) %>
						</td>
						...
						<td class="txt_l">
							<%if(!"".equals(data.get("filename"))){ %>
							<a href="/include/download.jsp?path=<%=uploadPath%>&vf=<%=data.get("filename")%>&af=<%=data.get("filename_org")%>" target="_blank"><%=Function.parseStr(data.get("filename_org"))%></a>
							<%} %>
							<% if(!"".equals(data.get("filename2"))){ %>
							<a href="/include/download.jsp?path=<%=uploadPath%>&vf=<%=data.get("filename2")%>&af=<%=data.get("filename2_org")%>" target="_blank"><%=Function.parseStr(data.get("filename2_org"))%></a>
							<% } %>
						</td>
						<td <%=targetUrl%>>
							<%=data.get("readno") %>
